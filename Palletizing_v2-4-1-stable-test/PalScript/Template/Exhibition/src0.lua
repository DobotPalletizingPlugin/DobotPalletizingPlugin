...
function PalletGetPose(PalletNumber, PalletPoint, PalletType)
    ...
    if (PalletType == 2) then
        PalletUserNum = PalletNumber.Coordinate.PartitionUserNum
    else
...
PartPickPoseFP = PalletGetPose(FirstPallet, FirstPallet.TeachPoint.TeachPartitionPickPoint, 2)
PartPlacePoseFP = PalletGetPose(FirstPallet, FirstPallet.TeachPoint.TeachPartitionPlacePoint, 0)
PickPoseFP =
{
    pose = { 0, 0, 0, 0, 0, 0 }
}
...
PartPickPoseSP = PalletGetPose(SecondPallet, SecondPallet.TeachPoint.TeachPartitionPickPoint, 2)
PartPlacePoseSP = PalletGetPose(SecondPallet, SecondPallet.TeachPoint.TeachPartitionPlacePoint, 0)
PickPoseSP =
{
    pose = { 0, 0, 0, 0, 0, 0 }
}
...
local function RobotGoHome()
    MovJ(HomePoint, {a = ToolAccWithoutBox, v = ToolSpeedWithoutBox, cp = 100})
end
...
---------------------------------------------------------------
--获取隔板高度
local function GetPartitionHeight(PalletNumber, PartitionPickPose)
    PartitionNum = ReadRobotModbus(FirstPallet.RegisterID.PartitionNum) --隔板计数
    if PalletNumber.PalletMode == 1 then
        if PartitionNum <= 0 then
            ErrorMessage.Code = 7
            Alarm("隔板已空")
            PartitionNum = PalletNumber.ProcessNum.PartitionNum
        end
    else
        if PalletNumber.ProcessNum.PartitionNum + 1 <= PartitionNum then
            ErrorMessage.Code = 7
            Alarm("隔板已满")
            PartitionNum = 1
        end
    ...
    local PickPoseOffset = RelPointUser(PartitionPickPose,
        { 0, 0, PalletNumber.ProcessNum.PartitionHeight * PartitionNum, 0, 0, 0 })
    ...
    if PalletNumber.PalletMode == 1 then
        if (PalletNumber.PalletNum.LayerCount >= PalletNumber.CompensateLayer)
            or ((PalletNumber.PalletNum.LayerCount == PalletNumber.CompensateLayer - 1)
                ...
                and (PalletNumber.Partition.Enable == false)) then
...
local function RzCompensate(PalletNumber, DestPose, NameStr)
    local OffSet = {}
    OffSet, PalletNumber.BoxProperty.Sucker =
        GetBoxProPerty(NameStr, Pallet, PalletNumber.PalletNum.NextBoxCount)
    if PalletNumber.BoxProperty.Sucker > 0 then
        PalletNumber.BoxProperty.Sucker = 1
    end
...
local function GetPointInfo(PalletNumber, CurrentPointType, CurrentIndex)
    ...
    if PalletNumber.Partition.Enable == true then
        ErrorMessage.PointInfo.Type = CurrentPointType.WithPartition
    else
        ErrorMessage.PointInfo.Type = CurrentPointType.NoPartition
    end

    if PalletNumber.PalletMode == 1 then
        if (PalletNumber.ProcessNum.BoxCount >= PalletNumber.PalletNum.LayerBoxNum)
            and (PalletNumber.PalletNum.LayerBoxNum > 0) and (PalletNumber.Partition.Enable == false) then
...
local function AdjustLiftingHeight(FallingHeight, RelativeHeight)
    if (RelativeHeight > 0) then
        MovJ(LiftingPoint, { a = ToolAccWithoutBox, v = ToolSpeedWithoutBox, cp = 100 })
...
local function ConveyorUserSwitch(PalletNumber, UserFlag, UserNum, NameStr)
    if (WorkingMode == 1) and (PalletNumber.Partition.Enable == false) then
        local SwitchUser =
        {
            [0] = function()
...
local function ToolSwitch(PalletNumber, ToolFlag, NameStr)
    local CurrentTool = {}
    if (PalletNumber.Partition.Enable == false)
        and (PalletSuckerFunction == 2)
        and (PalletNumber.BoxProperty.Sucker == 0) then
        local SwitchTool =
        {
            [0] = function()
...
---------------------------------------------------------------
--计算位置：放置位置
local function GetPlacePositon(PalletNumber, PartitionPickPose, PartitionPlacePose, NameStr)
...
local function GetAxisHeight(PalletNumber, DestPose, NameStr)
    ...
    --[[if PalletNumber.Partition.Enable == false then
        local LayerCount = 0
        if PalletNumber.PalletMode == 1 then
            if (PalletNumber.ProcessNum.BoxCount >= PalletNumber.PalletNum.LayerBoxNum)
        ...
        OffSet, Sucker = GetBoxProPerty(NameStr, Pallet, Index)
        ToolSwitch(PalletNumber, Sucker, NameStr)
    ...
    ExAxisHeight = ChooseTransPoint(PalletNumber, DestPose, ExAxisHeight,
        PalletNumber.OffsetHeight, 0, 1, ToolAccWithBox, ToolSpeedWithBox)
    ToolSwitch(PalletNumber, PalletNumber.BoxProperty.Sucker, NameStr)
...
local function CalPlaceBoxNum(PalletNumber, NameStr)
    ...
    if PalletNumber.PalletNum.NextBoxCount <= 1 then
        OffSet, Sucker = GetBoxProPerty(NameStr, Pallet, 1)
    else
        OffSet, Sucker = GetBoxProPerty(NameStr, Pallet, PalletNumber.PalletNum.NextBoxCount - 1)
    end
    if Sucker > 0 then
...
--------------------------------------------------------------
--启动带料箱动作
local function GetInitSuckerAction()
    DO(FirstPallet.TriLight.Yellow, OFF) --栈板三色灯
    DO(FirstPallet.TriLight.Green, OFF)
...
--------------------------------------------------------------
--启动带料箱状态
local function GetInitSuckerState()
    local SwitchTerminalTool =
    {
        [0] = function()
            if SuckerCfg.Dete.Mode == 1 then
                if (ToolDI(SuckerCfg.Dete.PE.A) == ON) and (CheckIORes(SuckerCfg.Port.Mode, SuckerCfg.Port.A) == ON) then
                    GetInitSuckerAction()
                end
            elseif SuckerCfg.Dete.Mode == 2 then
                if (DI(SuckerCfg.Dete.Vacuum.A) == ON) and (CheckIORes(SuckerCfg.Port.Mode, SuckerCfg.Port.A) == ON) then
                    GetInitSuckerAction()
                end
            else
                print("启动带料箱功能关闭！")
        ...
        [1] = function()
            if SuckerCfg.Dete.Mode == 1 then
                if ((ToolDI(SuckerCfg.Dete.PE.A) == ON) or (ToolDI(SuckerCfg.Dete.PE.B) == ON))
                    and (CheckIORes(SuckerCfg.Port.Mode, SuckerCfg.Port.A) == ON or CheckIORes(SuckerCfg.Port.Mode, SuckerCfg.Port.B) == ON) then
                    GetInitSuckerAction()
                end
            elseif SuckerCfg.Dete.Mode == 2 then
                if ((DI(SuckerCfg.Dete.Vacuum.A) == ON) or (DI(SuckerCfg.Dete.Vacuum.B) == ON))
                    and (CheckIORes(SuckerCfg.Port.Mode, SuckerCfg.Port.A) == ON or CheckIORes(SuckerCfg.Port.Mode, SuckerCfg.Port.B) == ON) then
                    GetInitSuckerAction()
                end
            else
    ...
    local switch_mode = SwitchTerminalTool[PalletSuckerFunction]
    if switch_mode then
        switch_mode()
    else
...
local function InitSucker()
    ...
    local SwitchSucker =
    {
        [0] = function()
            ...
            if PartitionSignalFunction then
                IORes(PartSignalMode, PartitionSignal, OFF)
            end
        ...
        [2] = function()
            ...
            Err, Communication.Sucker.Tcp.Socket = TCPCreate(false,
                Communication.Sucker.Tcp.Ip, Communication.Sucker.Tcp.Port) --创建TCP客户端
            print("Create TCP Client Success!")
            Err = TCPStart(Communication.Sucker.Tcp.Socket, 5)              --建立TCP连接
            if Err ~= 0 then
                ErrorMessage.Code = 8
                Alarm("吸盘初始化失败！")
            end
    ...
    local switch_mode = SwitchSucker[PalletSuckerFunction]

    if switch_mode then
        switch_mode()
    else
        print("The SwitchSucker is error, please check it!")
        Pause()
    end
...
---------------------------------------------------------------
--打开吸盘
local function OpenSucker(PalletNumber)
    local CurrentTool = {}
    local CurrentToolNum = 0
    if (PalletSuckerFunction == 2)
        and (PalletNumber.BoxProperty.Sucker == 0)
        and (PalletNumber.Partition.Enable == false) then
    ...
    local SwitchOpenSucker =
    {
        [0] = function()
            if PalletSuckerFunction == -1 then
                ...
                IORes(SuckerCfg.Port.Mode, SuckerCfg.Port.A, ON)                                               --数字输出控制吸盘吸气
            end
        ...
        [2] = function()
            if PalletSuckerFunction == -1 then
                ...
                if PartitionSignalFunction then
                    IORes(PartSignalMode, PartitionSignal, ON)
                else
                    if PalletSuckerFunction == 2 then
                        IORes(SuckerCfg.Port.Mode, SuckerCfg.Port.A, ON) --数字输出控制吸盘吸气
                        IORes(SuckerCfg.Port.Mode, SuckerCfg.Port.B, ON)
                    else
                        IORes(SuckerCfg.Port.Mode, SuckerCfg.Port.A, ON) --数字输出控制吸盘吸气
    ...
    if PalletNumber.Partition.Enable == true then
        ...
        ModeFlag = PalletNumber.BoxProperty.Sucker
    end
    local switch_mode = SwitchOpenSucker[ModeFlag]

    if switch_mode then
        switch_mode()
    else
        print("The SwitchOpenSucker is error, please check it!")
        Pause()
    end
end
...
---------------------------------------------------------------
--关闭吸盘
local function CloseSucker(Sucker)
    local CurrentTool = {}
    ...
    if (PartitionSignalFunction == true)
        and (FirstPallet.Partition.Enable == true or SecondPallet.Partition.Enable == true) then
        Sucker = 2
    end
    local switch_mode = SwitchCloseSucker[Sucker]

    if switch_mode then
        switch_mode()
    else
        print("The SwitchCloseSucker is error, please check it!")
        Pause()
    end
end
...
---------------------------------------------------------------
--初始化隔板数量
local function InitPartition()
    if Pallet == 0 then
        PartitionNum = ReadRobotModbus(FirstPallet.RegisterID.PartitionNum) --隔板计数
        if (PartitionNum > FirstPallet.ProcessNum.PartitionNum) or (PartitionNum <= 0) then
            if (FirstPallet.PalletMode == 1) then
                WriteRobotModbus(FirstPallet.ProcessNum.PartitionNum, FirstPallet.RegisterID.PartitionNum)
            else
                WriteRobotModbus(1, FirstPallet.RegisterID.PartitionNum)
            end
        end
    else
        PartitionNum = ReadRobotModbus(FirstPallet.RegisterID.PartitionNum) --隔板计数
...
local function PickPallet(PalletNumber, DestPose, LiftingHeight)
    ...
    if PalletNumber.Partition.Enable == true then
        PartPoint = PositiveKin(PartSafePoint,
            {
                user = PalletNumber.Coordinate.UserNum,
                tool = PalletNumber.Coordinate.ToolNum
            })
    ...
    Wait(Time.Pick.Pre)
    MovJ(DestJointOffset, { a = ToolAccWithoutBox, v = ToolSpeedWithoutBox, cp = 100 })
    MovL(PickJoint, { a = ToolAccWithoutBox, v = ToolSpeedWithoutBox, cp = 100 }) --运动到抓取点
    OpenSucker(PalletNumber)
    Wait(Time.Pick.Post)
...
local function GetStandbyPose()
    ...
    if switch_mode then
        ...
        if PalletObstacleFunc == 1 and (FirstPallet.TransPointMode == 1 or SecondPallet.TransPointMode == 1) then
            MovL(DestJointOffset, { a = ToolAccWithoutBox, v = ToolSpeedWithoutBox, cp = 100 }) --运动到层过渡点
        else
            MovJ(DestJointOffset, { a = ToolAccWithoutBox, v = ToolSpeedWithoutBox, cp = 100 })
        end
    else
...
local function PlacePallet(PalletNumber, DestPose, LiftingHeight, NameStr)
    ...
    MovL(StandyPoint, { a = ToolAccWithoutBox, v = ToolSpeedWithoutBox, cp = 100 })
    ChooseTransPoint(PalletNumber, DestPose, LiftingHeight,
        OffHeight, 0, 0, ToolAccWithBox, ToolSpeedWithBox)
    MovL(tempDestJoint, { a = ToolAccWithBox, v = ToolSpeedWithBox, cp = 100 }) --运动到放置过渡点
    MovL(tempPlaceOffsetJoint, { a = PlaceAcc, v = PlaceSpeed, cp = 100 })      --运动到放置点正上方
    MovL(tempPlaceJoint, { a = PlaceAcc, v = PlaceSpeed, cp = 100 })
    if PalletNumber.Partition.Enable == false then
        CalPlaceBoxNum(PalletNumber, NameStr) --执行托盘码垛计数
    else
        PartitionNum = PartitionNum - 1
        WriteRobotModbus(PartitionNum, FirstPallet.RegisterID.PartitionNum)
    end
    CloseSucker(PalletNumber.BoxProperty.Sucker)
    
    MovL(tempPlaceOffsetJoint, { a = ToolAccWithoutBox, v = ToolSpeedWithoutBox, cp = 100 }) --运动到放置点正上方
    MovL(tempDestJoint, { a = ToolAccWithoutBox, v = ToolSpeedWithoutBox, cp = 100 })        --运动到放置过渡点
    PartCoordinateFlag = 0
    ChooseTransPoint(PalletNumber, DestPose, LiftingHeight,
        OffHeight, 1, 0, ToolAccWithoutBox, ToolSpeedWithoutBox)
    MovJ(StandyPoint, { a = ToolAccWithoutBox, v = ToolSpeedWithoutBox, cp = 100 })
    local CurrentPose = { pose = { 0, 0, 0, 0, 0, 0 } }
    CurrentPose = GetPose()
    ToolSwitch(PalletNumber, 1, NameStr)
    --GetStandbyPose()
...
local function PickDePalletCal(PalletNumber, DestPose, LiftingHeight, NameStr)
    ...
    local tempPlaceJoint = SwitchPointMode(PalletNumber, CopyPlacePos)
    ChooseTransPoint(PalletNumber, DestPose, LiftingHeight,
        OffHeight, 0, 0, ToolAccWithoutBox, ToolSpeedWithoutBox)
    MovL(tempDestJoint, { a = ToolAccWithoutBox, v = ToolSpeedWithoutBox, cp = 100 })        --运动到放置过渡点
    MovL(tempPlaceOffsetJoint, { a = ToolAccWithoutBox, v = ToolSpeedWithoutBox, cp = 100 }) --运动到放置点正上方
    MovL(tempPlaceJoint, { a = ToolAccWithoutBox, v = ToolSpeedWithoutBox, cp = 100 })
    OpenSucker(PalletNumber)
    MovL(tempPlaceOffsetJoint, { a = ToolAccWithBox, v = ToolSpeedWithBox, cp = 100 }) --运动到放置点正上方
    MovL(tempDestJoint, { a = ToolAccWithBox, v = ToolSpeedWithBox, cp = 100 })        --运动到放置过渡点
    if PalletNumber.Partition.Enable == true then
        PartCoordinateFlag = 2
    end
    ChooseTransPoint(PalletNumber, DestPose, LiftingHeight,
        OffHeight, 1, 0, ToolAccWithBox, ToolSpeedWithBox)
    ToolSwitch(PalletNumber, 1, NameStr)
end
...
local function PlaceDePallet(PalletNumber, DestPose, LiftingHeight, NameStr)
    ...
    PalletNumber.NearFlag = false
    if PalletObstacleFunc == 1 and PalletNumber.TransPointMode == 1 then
        MovL(PlaceJointOffset, { a = ToolAccWithBox, v = ToolSpeedWithBox, cp = 100 })
    else
        MovJ(PlaceJointOffset, { a = ToolAccWithBox, v = ToolSpeedWithBox, cp = 100 })
    end
    PalletNumber.NearFlag = true
    PlaceJoint = PalletGetPose(PalletNumber, PlacePoint, 1)
    PalletNumber.NearFlag = false
    MovL(PlaceJoint, { a = ToolAccWithBox, v = ToolSpeedWithBox, cp = 100 }) --运动到抓取点
    if PalletNumber.Partition.Enable == false then
    ...
    MovL(PlaceJointOffset, { a = ToolAccWithoutBox, v = ToolSpeedWithoutBox, cp = 100 }) --运动到抓取点上方
    local CurrentPose = { pose = { 0, 0, 0, 0, 0, 0 } }
    CurrentPose = GetPose()
    PrePallet = Pallet
    PrePoseHeight = CurrentPose.pose[3] + ExAxisLiftingHeight
...
local function PlaceDePalletBoxAction(PalletNumber, DestPose, NameStr)
    ...
    if PalletNumber.Partition.Enable == true then
        local PartPoint = PositiveKin(PartSafePoint,
            {
                user = PalletNumber.Coordinate.UserNum,
                tool = PalletNumber.Coordinate.ToolNum
            })
