...
local function InitFSM()
    ...
    if (FirstPallet.StateValue.Enable == 1)
        ...
        if (PalletBeInPlaceOKButton == false or SimulateMode == 1) then
            ...
            if (FirstPallet.State.Replace == false or SecondPallet.State.Replace == false) then
...
local function InitLifting()
    if SimulateMode == 1 then
        ...
        elseif (Communication.Lifting.Mode == 1) then
    ...
    local SwitchCommMode =
    {
        [0] = function()
            repeat
                ...
                Wait(100)
            until ((Res == CLH) or (Res == CLH + 1) or (Res == CLH - 1))
        end,
        [1] = function()
            ...
            repeat
                ...
                Wait(100)
            until ((Res[1] == RLH) or (Res[1] == RLH + 1) or (Res[1] == RLH - 1))
        end,
        [2] = function()
            repeat
                ...
                LogInfo("Lifting motion position is %s mm! ", Res)
                Wait(100)
            until ((Res == math.ceil(CLH)) or (Res == math.ceil(CLH + 1)) or (Res == math.ceil(CLH - 1)))
        end
    }
    local switch_mode = SwitchCommMode[Communication.Lifting.Mode]
    if switch_mode then
...
local function FilmMotion()
    ...
    if FilmFunction == true then
        if (DI(FilmDI) == ON)
            and (FilmDone == true)
            and (CheckIORes(SuckerCfg.Port.Mode, SuckerCfg.Port.A) == OFF)
            and (CheckIORes(SuckerCfg.Port.Mode, SuckerCfg.Port.B) == OFF) then
            FilmDone = false
...
local function UpdateData(PalletNumber, CPoint)
    ...
    if (CPoint.Paras.Mode == MotionType.Part) then
        if (PalletNumber.Mode == WorkType.Pallet) then
            FirstPallet.PalletNum.RePartNum = FirstPallet.PalletNum.RePartNum - 1
        else
            FirstPallet.PalletNum.RePartNum = FirstPallet.PalletNum.RePartNum + 1
        end
        WritePartNum(PalletNumber)
    ...
    if (PalletNumber.Mode == WorkType.Pallet) then
        ...
        if (CPoint.Paras.Mode == MotionType.Part) then
            ...
            CPartPick.pose[3] = CPartPick.pose[3] - CPoint.Paras.LH +
                PalletNumber.ProcessNum.PartitionHeight * FirstPallet.PalletNum.RePartNum
