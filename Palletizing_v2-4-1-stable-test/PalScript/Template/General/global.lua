...
function EWLGetPosition()
    ...
    return Err, tonumber(EHeightResult[3])
end
...
function callBackStopLifting()
    if PalletLiftingFunction == true then
        ...
        LogWarn("Script pause and lifting stop moving!")
...
function WritePartNum(PalletNumber)
    WriteRobotModbus(FirstPallet.PalletNum.RePartNum, FirstPallet.RegisterID.RePartNum)
    SecondPallet.PalletNum.RePartNum = FirstPallet.PalletNum.RePartNum
...
function ReadPalletNum(PalletNumber)
    if StorageMode == 1 then
        ...
        FirstPallet.PalletNum.RePartNum = ReadRobotModbus(FirstPallet.RegisterID.RePartNum)
    else
        local TData = {}
        if PalletNumber.Pallet == Left then
        ...
        PalletNumber.PalletNum.RePartNum = TData.RePartNum
        if ((StateMachine == FSMType.DLR or StateMachine == FSMType.SLR) and (PalletNumber.Pallet == Right)) then
            if (PalletNumber.Mode == WorkType.Pallet) then
                if (FirstPallet.PalletNum.RePartNum > SecondPallet.PalletNum.RePartNum
                        and FirstPallet.PalletNum.RePartNum ~= FirstPallet.ProcessNum.PartitionNum) then
                    FirstPallet.PalletNum.RePartNum = SecondPallet.PalletNum.RePartNum
                end
            else
                if (FirstPallet.PalletNum.RePartNum < SecondPallet.PalletNum.RePartNum
                        and FirstPallet.PalletNum.RePartNum ~= 0) then
                    FirstPallet.PalletNum.RePartNum = SecondPallet.PalletNum.RePartNum
                end
            end
        elseif (StateMachine == FSMType.SR) then
            FirstPallet.PalletNum.RePartNum = SecondPallet.PalletNum.RePartNum
        end
    end
    ...
    if PalletNumber.PalletNum.LayerCount < 1 then
        ...
        FirstPallet.PalletNum.RePartNum = FirstPallet.ProcessNum.PartitionNum
    end
    if (PalletNumber.Partition.Enable == true) then
        if (PalletNumber.Mode == WorkType.Pallet) then
            if FirstPallet.PalletNum.RePartNum <= 0 then
                -- 仿真模式，不需要隔板数量报警
                if SimulateMode == 1 then
                    FirstPallet.PalletNum.RePartNum = FirstPallet.ProcessNum.PartitionNum
                    return
            ...
            if (FirstPallet.PalletNum.RePartNum > FirstPallet.ProcessNum.PartitionNum) then
                FirstPallet.PalletNum.RePartNum = FirstPallet.ProcessNum.PartitionNum
            end
        else
            if (FirstPallet.PalletNum.RePartNum >= FirstPallet.ProcessNum.PartitionNum) then
                -- 仿真模式，不需要隔板数量报警
                if SimulateMode == 1 then
                    FirstPallet.PalletNum.RePartNum = 0
                    return
                end
        ...
        if not ((StateMachine == FSMType.DLR or StateMachine == FSMType.SLR) and (PalletNumber.Pallet == Left)) then
            WritePartNum(PalletNumber)
        end
        if PalletNumber.Pallet == Left then
    ...
    LogInfo(
        ...
        PalletNumber.PalletNum.AddBoxCount, PalletNumber.Partition.Place, FirstPallet.PalletNum.RePartNum)
end
---------------------------------------------------------------
    ...
    if (PalletNumber.StateValue.Enable == 1) then
        if (PalletBeInPlaceOKButton == true) then
            PalletNumber.State.Replace = (DI(PalletNumber.InPlaceA) == ON)
                ...
                and ((DI(PalletNumber.InPlaceOK) == ON) or (PalletNumber.State.InPlaceOK == true))
            if (PalletNumber.State.Replace == true) then
                ...
                if (StateMachine == FSMType.SLR or StateMachine == FSMType.DLR) then
                    ...
                    elseif (PalletNumber.Pallet == Right) then
...
function DropSignalDete(PalletNumber, DeteState, SuckerPort, Mode)
    if (Mode == DropType.Norm) then
        if (PalletNumber.Partition.Mode == MotionType.Part) and (PartCfg.Enable == true) then
            if (DeteState == OFF) and (CheckIORes(PartCfg.Port.Mode, PartCfg.Port.A) == ON) then
            ...
            if (DeteState == OFF) and (CheckIORes(SuckerCfg.Port.Mode, SuckerPort) == ON) then
                Wait(Time.DropDetection)
                if (DeteState == OFF) and (CheckIORes(SuckerCfg.Port.Mode, SuckerPort) == ON) then
                    TriLightStatus(PalletNumber, Light.Red.On)
