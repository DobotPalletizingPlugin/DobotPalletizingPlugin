...
local function ReadProductionTime()
    ...
    if StorageMode == 1 then
        ...
        CapacityBox = ReadRobotModbus(Capacity.RegisterID.Box)
    else
        ...
        local CapacityData = GetVal("PalletCapacity")
        if (CapacityData == nil) then
            CapacityBox = 0
        else
            CapacityBox = CapacityData.Box
        end
    end
