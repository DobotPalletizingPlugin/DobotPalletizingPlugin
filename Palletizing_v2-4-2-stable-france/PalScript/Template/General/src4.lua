...
local function ReadProductionTime()
    ...
    LogInfo("Données de capacité initialisées : heures = %s, minutes = %s, secondes = %s",
    ...
    if ((FirstPallet.StateValue.Status == StateType.Run) and (FirstPallet.State.Replace == true))
        ...
        if Communication.Lifting.TimesPerHour > Communication.Lifting.MaxtimesPerHour then
            ...
            if (TotalTime - TotalTimeStart) < 3600 then
                TotalTimeStart = TotalTime
                Alarm("Erreur du nombre de mouvements de la colonne élévatrice !", ErrorMessage.Type.LiftingErr)
            end
        end
...
local function DetePalletFSM()
    local SwitchFSM =
    {
        [FSMType.IDLE] = function()
            LogWarn("La machine d’état de détection des palettes est inactive !")
        end,
        [FSMType.SL] = function()
    ...
    if switch_conveyor then
        ...
        Alarm("DetePalletFSM est incorrect !", ErrorMessage.Type.WorkingDataErr)
    end
end
---------------------------------------------------------------
---------------------------------------------------------------
