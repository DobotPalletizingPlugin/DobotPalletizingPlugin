...
PartSensorCfg =                                                  --隔板余量检测传感器配置
{
    Enable = false,                                              --false：关闭，true：启用
    Port =
    {
        Mode = 0, --IO类型, 0：通用IO，1：末端IO
        A = 22    --隔板检测DI，现场接线确认后修改
    },
...
------------------------------------------------------------------------------------------------
BIBConveyorCfg = -- BIB项目专用输送线控制配置：两个实体cartons作为一个逻辑box取料
{
    ...
    Sensor =
    {
        ...
        B3 = { Mode = 0, A = 19 }  --DI19：第三个carton到达检测
    },
