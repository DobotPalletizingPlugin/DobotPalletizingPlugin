...
BuzzerIO = 21                                                    --蜂鸣器输出信号：本项目现场使用DO21。
-- Buzzer output signal used for alarms, operator waiting states, or pallet-change prompts.
-- L’APAVE压缩空气安全检查配置。
-- L’APAVE compressed-air safety-check configuration.
AirPressureCfg =
{
    Enable = true,
    PresenceDI = 22,
    BuzzerDuration = 10000 -- ms，L’APAVE要求10至15秒
}
-- 独立隔板吸附控制。
...
function CreateQueue()
    ...
    function Queue:_acquireLock()
        ...
        while (self._locked) do
            if (os.clock() - startTime) > 5 then
                Alarm("Délai d’attente dépassé pour le verrouillage de la file.", ErrorMessage.Type.QueueTimeout)
            end
            Wait(1) -- 短暂等待，让出执行权
    ...
    function Queue:Push(Value)
        if (Value == nil) then
            Alarm("Erreur lors de l’ajout d’un point dans la file !", ErrorMessage.Type.PointErr)
        end
        self:_acquireLock()
    ...
    function Queue:Pop()
        ...
        if self:_isEmpty() then
            self:_releaseLock()
            LogWarn("Erreur lors du retrait d’un point de la file !")
            return nil
        end
    ...
    function Queue:Peek()
        ...
        if self:_isEmpty() then
            self:_releaseLock()
            LogWarn("Erreur lors de la lecture du premier point de la file !")
            return nil
        end
        local Value = self._data[self._head]
        self:_releaseLock()
...
function DeepClear(T)
    if type(T) ~= "table" then
        LogWarn("Le type du point n’est pas une table !")
        return
    end
...
function GetTableKey(T, Value)
    ...
    LogInfo("Valeur : %s", Value)
    LogInfoTable("table:", T)
    Alarm("Erreur GetTableKey !", ErrorMessage.Type.WorkingDataErr)
...
function EWLGetPosition()
    ...
    if (Res == nil) then
        for n = 1, 10 do
            LogWarn("Données EWLGetPosition : %s ; données séparées : %s !", EHeight, EHeightResult)
            TCPWrite(Communication.Lifting.Brand.EWELLIX.Tcp.Socket,
                Communication.Lifting.Brand.EWELLIX.Command.GetPosition)
            Err, EHeight = TCPRead(Communication.Lifting.Brand.EWELLIX.Tcp.Socket, 5, "string")
            EHeightResult, ResultLength = Split(EHeight, ",")
        ...
        LogError("Données EWLGetPosition : %s ; données séparées : %s !", EHeight, EHeightResult)
        Alarm("Erreur EWLGetPosition !", ErrorMessage.Type.LiftingDataErr)
    end
    return Err, Res
end
...
function SV660CRun(Postion)
    local CurrentId = Communication.Lifting.Brand.GeMinG.ModbusRTU.SlaveId
    if Postion == nil then
        Alarm("La position est nulle !", ErrorMessage.Type.LiftingDataErr)
    end
...
function LINAKRun(Postion)
    ...
    repeat
        ...
        LogInfo("Colonne LINAK : code d’erreur %s, état %s", Err, Status)
...
function TriLightStatus(PalletNumber, State)
    ...
    if switch_State then
        ...
        Alarm("L’état du voyant tricolore est incorrect. Veuillez le vérifier !", ErrorMessage.Type.WorkingDataErr)
    end
end
---------------------------------------------------------------
--回调函数
...
function callBackStopLifting()
    if PalletLiftingFunction == true then
        ...
        if switch_mode then
            ...
            Alarm("Le mode de pause est incorrect. Veuillez le vérifier !", ErrorMessage.Type.WorkingDataErr)
        end
        LogWarn("Mise en pause de la colonne élévatrice !")
    ...
    LogInfo("État des signaux : LWorkState = %s, RWorkState = %s", GetTableKey(StateType, GetVal("LWorkState")),
        GetTableKey(StateType, GetVal("RWorkState")))
    LogInfo("Script mis en pause !")
end
...
function callContinuteMotion()
    LogInfo("Reprise du script et du mouvement de la colonne élévatrice !")
    ...
    LogInfo("État des signaux : LRSignal = %s, RRSignal = %s, LButton = %s, RButton = %s", FirstPallet.State.SReset,
...
function ClearError()
    ...
    LogInfo("Initialisation des messages d’erreur réussie !")
end
---------------------------------------------------------------
--报警
-- Alarm.
function Alarm(Message, ErrorCode)
    ...
    if (PalletLiftingFunction == true) and (Communication.Lifting.CMaxDis ~= 0) then
        ...
        {
            [LiftingType.EWELLIX] = function()
                LogInfo("Arrêt de la colonne EWELLIX")
                EWLEnable(0)
            end,
            [LiftingType.GeMinG] = function()
                LogInfo("Arrêt de la colonne GeMinG")
                SV660CEnable(0)
            end,
            [LiftingType.ZT3ILC] = function()
                LogInfo("Arrêt de la colonne ZT3ILC")
                ZC01Enable(0)
            end,
            [LiftingType.LINAK] = function()
                LogInfo("Arrêt de la colonne LINAK")
                LINAKStop()
            end
        }
        ...
        if switch_mode then
            ...
            LogError("Le mode d’arrêt est incorrect. Veuillez le vérifier !")
            Halt()
        end
    end
    CommitErrorMsg(ErrorMessage.Code)
    ...
    LogError("Code d’erreur : %s, %s", ErrorMessage.Code, Message)
    if (ErrorMessage.Code == ErrorMessage.Type.PartErr) then
        LogInfo("Mise en pause du mouvement du robot")
        Pause() --暂停机器人当前运动
        -- Pause the robot's current motion.
        ClearError()
    else
        LogInfo("Arrêt du mouvement du robot")
        Halt()  --立即停止机器人当前运动
...
function InitModbus()
    ...
    if Err == 0 then
        LogInfo("Connexion Modbus réussie !")
        ClearError()
        Communication.Controller.Modbus.LinkState = true
    else
        while true do
            Communication.Controller.Modbus.LinkState = false
            Alarm("Échec de la connexion Modbus !", ErrorMessage.Type.IPErr)
        end
    end
end
---------------------------------------------------------------
...
function ReadRobotModbus(Address)
    ...
    while buffer[1] == nil do
        LogWarn("Les données du registre Modbus sont nulles !")
...
function WriteRobotModbus(Data, Address)
    ...
    if (Data == nil or Address == nil) then
        LogError("Adresse : %s, les données sont nulles !", Address)
        Alarm("Les données sont nulles !", ErrorMessage.Type.WorkingDataErr)
    end
    for i = 1, 20, 1 do
        ...
        if (i == 20) then
            ...
            Alarm("Échec de l’écriture des données Modbus du robot !", ErrorMessage.Type.WorkingDataErr)
        end
        Wait(4)
    end
end
...
function SyncPartitionRemainBySensor(PalletNumber, ForceLog)
    ...
    if (PalletNumber.Partition.RePartNum ~= TargetPartNum) or (ForceLog == true) then
        ...
        if SensorState == ON then
            LogInfo("Le capteur de présence des intercalaires DI%s est actif. Restauration du nombre d’intercalaires par défaut et écriture Modbus : %s.",
...
function ReadPalletNum(PalletNumber)
    ...
    if PalletNumber.PalletNum.NextBoxCount < PalletNumber.PalletNum.AddBoxCount then
        LogInfo("Données de travail de la palette %s : NextBoxCount = %s, AddBoxCount = %s",
            (PalletNumber.Pallet == Left and "gauche" or "droite"),
            PalletNumber.PalletNum.NextBoxCount, PalletNumber.PalletNum.AddBoxCount)
        Alarm("ReadPalletNum : AddBoxCount est incorrect !", ErrorMessage.Type.WorkingDataErr)
    end
        ...
        if (PalletNumber.Mode == WorkType.Pallet) then
            if (PalletNumber.Partition.RePartNum <= 0) then
                ...
                if IsPartitionRemainSensorMode(PalletNumber) == true then
                    LogInfo("Le nombre d’intercalaires restants est contrôlé par un capteur DI. L’alarme RePartNum vide enregistrée est ignorée avant la synchronisation du capteur.")
                else
                    -- 仿真模式，不需要隔板数量报警
    ...
    LogInfo(
        "Données de travail de la palette %s : LayerCount = %s, NextBoxCount = %s, RemainBoxNum = %s, LayerBoxNum = %s, AddBoxCount = %s, PartPlace = %s, PartitionNum = %s",
        (PalletNumber.Pallet == Left and "gauche" or "droite"),
...
function ReadProductionData()
    ...
    LogInfo("Données de capacité initialisées : palettes = %s, cartons = %s", Capacity.Num.RePallet, Capacity.Num.ReBox)
end
...
function ExecuteSafeModule(PalletNumberA, PalletNumberB)
    ...
    if (ModuleA.Grating.Enable == true) then
        while (CheckDIRes(ModuleA.Grating.Port.Mode, ModuleA.Grating.Port.A) == OFF) do
            LogWarn("Le module de barrière immatérielle est déclenché !")
            Pause()
        end
        return
    end
...
function CheckCompressedAirPresence()
    if (AirPressureCfg == nil) or (AirPressureCfg.Enable ~= true) then
        return true
    end

    local PresenceDI = tonumber(AirPressureCfg.PresenceDI) or 22
    local AirState = DI(PresenceDI)

    -- 在与隔板传感器相同的ACK/初始化阶段明确记录气源检查结果。
    if AirState == ON then
        LogInfo("Présence d'air comprimé confirmée. DI%s = ON.", PresenceDI)
        return true
    end

    local Message = "Absence d'air comprimé. Veuillez vérifier l'alimentation pneumatique."
    local BuzzerDuration = tonumber(AirPressureCfg.BuzzerDuration) or 10000
    if BuzzerDuration < 10000 then
        BuzzerDuration = 10000
    elseif BuzzerDuration > 15000 then
        BuzzerDuration = 15000
    end

    LogError("%s DI%s = OFF.", Message, PresenceDI)
    ...
    Alarm(Message, ErrorMessage.Type.AirErr)
    ...
    if (Mode == DropType.Norm) then
        if (PalletNumber.Partition.Mode == MotionType.Part) and (PartCfg.Enable == true) then
            if (DeteState == OFF) and (CheckDORes(PartCfg.Port.Mode, PartCfg.Port.A) == ON) then
                ...
                if (DeteState == OFF) and (CheckDORes(PartCfg.Port.Mode, PartCfg.Port.A) == ON) then
                    ...
                    Alarm("Chute d’intercalaire détectée !", ErrorMessage.Type.DropErr)
                end
            end
        else
            if (DeteState == OFF) and (CheckDORes(SuckerCfg.Port.Mode, SuckerPort) == ON) then
                ...
                if (DeteState == OFF) and (CheckDORes(SuckerCfg.Port.Mode, SuckerPort) == ON) then
                    ...
                    -- Use digital output to turn off the sucker.
                    Alarm("Chute de carton détectée.", ErrorMessage.Type.DropErr)
                end
            end
        end
    else
        if (PartCfg.Enable == true) then
            if (DeteState == ON) and (CheckDORes(PartCfg.Port.Mode, PartCfg.Port.A) == ON) then
                ...
                Alarm("Carton détecté sur l’outil TCP !", ErrorMessage.Type.StartErr)
            end
        end
...
----------------------------------------------------------------
-- 本项目普通箱体双路掉料检测状态。
-- Project-specific dual-channel box-drop detection state.
-- DI23和DI24只检测机器人末端抓取的普通箱体，不用于隔板动作。
-- DI23 and DI24 monitor normal boxes held by the robot; they are not used for partition motion.
...
local function ResetBoxDropDetectionState()
    ...
    BoxDropStartTime = 0
    BoxDropAlarmed = false
end

-- 普通箱体双路掉料检测。
-- Dual-channel drop detection for normal boxes.
-- MotionMode由src2传入；只有MotionType.Norm才检查DI23/DI24。
-- MotionMode is supplied by src2; DI23/DI24 are checked only for MotionType.Norm.
-- 防抖使用Systime()，不阻塞src2状态线程；延时继续读取前端写入的Time.DropDetection。
-- Debouncing uses Systime() without blocking src2; the delay remains controlled by Time.DropDetection from the UI.
function BoxDropSignalDete(MotionMode)
    -- 仿真环境没有真实DI23/DI24，不执行现场掉料检测。
    if SimulateMode == 1 then
        ResetBoxDropDetectionState()
        return
    end

    -- 隔板动作、无当前动作或其它动作不使用DI23/DI24。
    if MotionMode ~= MotionType.Norm then
        ResetBoxDropDetectionState()
        return
    end

    -- 前端虽然配置为单吸，但本项目实际由一组吸盘输出抓取两个箱体。
    -- 只有吸盘输出开启时，DI23/DI24失去信号才视为掉料。
    ...
    local DropStateA = DI(23)
    local DropStateB = DI(24)

    -- 两路信号正常时清除之前的防抖计时。
    if (DropStateA == ON) and (DropStateB == ON) then
        ResetBoxDropDetectionState()
        return
    end

    local NowTime = Systime()
    local DelayTime = tonumber(Time.DropDetection) or 0
    if DelayTime < 0 then
        DelayTime = 0
    end

    -- 第一次发现任意一路OFF时只记录起始时间，不阻塞线程。
    if BoxDropDetecting == false then
        BoxDropDetecting = true
        BoxDropStartTime = NowTime
        if DelayTime > 0 then
            return
        end
    elseif (NowTime - BoxDropStartTime) < DelayTime then
        return
    end

    -- 达到前端设置的延时后，重新读取真实DI和吸盘输出，避免使用旧值误报警。
    DropStateA = DI(23)
    DropStateB = DI(24)
    SuckerState = CheckDORes(SuckerCfg.Port.Mode, SuckerCfg.Port.A)
    ...
    if (SuckerState == ON)
        ...
        if BoxDropAlarmed == false then
            ...
            -- Keep the sucker output on and trigger the alarm to stop robot motion.
            Alarm("Chute de carton détectée.", ErrorMessage.Type.DropErr)
        end
    else
        ResetBoxDropDetectionState()
...
function DropDete(PalletNumber, Mode)
    ...
    if switch_mode then
        ...
        Alarm("Le signal de chute est incorrect !", ErrorMessage.Type.WorkingDataErr)
    end
end
---------------------------------------------------------------
--吸盘输出信号类型
