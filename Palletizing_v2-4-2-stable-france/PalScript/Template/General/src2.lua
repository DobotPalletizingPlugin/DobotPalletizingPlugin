...
local function InitPallet(PalletNumber)
    if (PalletNumber.Pallet == Left) then
        Pallet = Right
        LogInfo("Basculement de la palette active vers la droite !")
    else
        Pallet = Left
        LogInfo("Basculement de la palette active vers la gauche !")
    end
...
local function InitWorkingData(PalletNumber)
    ...
    elseif PalletNumber.Mode == WorkType.Depallet then
        ...
        LogError("Mode de la palette : %s", PalletNumber.Mode)
        Alarm("Le mode de la palette est incorrect !", ErrorMessage.Type.WorkingDataErr)
    end
end
---------------------------------------------------------------
-- 更新码垛参数
...
local function InitData(PalletNumber)
    if (PalletNumber.State.Replace == true) and (PalletNumber.State.Init == false) then
        ...
        PalletNumber.ProcessNum.TotalBoxNum = GetBoxCnt(PalletName, PalletNumber.Pallet)
        LogInfo("Données de travail de la palette %s : TotalBoxNum = %s",
            (PalletNumber.Pallet == Left) and "gauche" or "droite", PalletNumber.ProcessNum.TotalBoxNum)
        if (PalletNumber.Mode == WorkType.Pallet) then
        ...
        LogInfo("Données de travail de la palette %s : BoxCount = %s",
            (PalletNumber.Pallet == Left) and "gauche" or "droite", PalletNumber.ProcessNum.BoxCount)
        ...
        LogInfo("Initialisation des données de travail de la palette %s terminée !", (PalletNumber.Pallet == Left) and "gauche" or "droite")
    end
end
---------------------------------------------------------------
--周期性循环检查
...
local function CheckPallet(PalletNumber)
    ...
    if (PalletNumber.State.SReset == true) then
        ...
        LogInfo("Remplacement de la palette %s terminé !", (PalletNumber.Pallet == Left and "gauche" or "droite"))
    end
end
...
local function ChildProcessAction(PalletNumber)
    ...
    if (PalletNumber.State.LosePallet == false) and (PalletNumber.State.Replace == false) then
        ...
        LogWarn("Perte de la palette %s détectée !", (PalletNumber.Pallet == Left) and "gauche" or "droite")
        TriLightStatus(PalletNumber, Light.Yellow.On)
    end
    ...
    -- 普通箱体由全机器人唯一的DI23/DI24检测函数处理。
    -- 隔板使用独立距离传感器，不执行标准掉料检测。
end
----------------------------------------------------------------
-- 获取机器人当前动作类型，仅用于区分普通箱体和隔板。
-- It does not read or process pallet-presence signals.
local function GetCurrentRobotMotionMode()
    if Pallet == Left then
        return FirstPallet.Partition.Mode
    elseif Pallet == Right then
        return SecondPallet.Partition.Mode
    end
    return nil
end
----------------------------------------------------------------
...
local function PalletStateCheck(PalletNumber)
    if (((DI(PalletNumber.InPlaceA) == ON) and (DI(PalletNumber.InPlaceB) == ON)) == false) then
        ...
        -- pallet not detected
        LogWarn("Palette %s introuvable !", (PalletNumber.Pallet == Left) and "gauche" or "droite")
    else
...
local function ExecuteDeteData(PalletNumber)
    ...
    -- 每个状态周期重新读取DI23和DI24；内部使用Systime()完成非阻塞防抖。
    -- 当前动作是隔板时，BoxDropSignalDete会清除计时并跳过双DI检测。
    BoxDropSignalDete(GetCurrentRobotMotionMode())
end
---------------------------------------------------------------
--获取工作状态
local function DeteDataFSM()
    local SwitchFSM =
    {
        [FSMType.IDLE] = function()
            LogWarn("La machine d’état DeteData est inactive !")
        end,
        [FSMType.SL] = function()
            while true do
    ...
    if switch_mode then
        ...
        Alarm("DeteDataFSM est incorrect !", ErrorMessage.Type.WorkingDataErr)
    end
end
---------------------------------------------------------------
---------------------------------------------------------------
