...
local function CreatePalletData(PalletType)
    return {
        ...
        PartTransTeachPose = {}, --隔板示教过渡点pose缓存：由joint+当前User/Tool重新正解，避免Tool Rz=90时姿态差90度
        Pallet = PalletType,
        User = 0,
        Tool = {
            Conc = 0,    --同心工具号
...
local function GetTeachPoint(PalletNumber, CData)
    if (CData.Init == false) then
        ...
        if (PalletNumber.Partition.Enable == true) then
            ...
            --保存隔板示教过渡点J6和pose。
            --注意：客户工具坐标系可能带Rz=90。这里优先用joint + 当前User/Tool重新正解pose，
            --后续只允许改Z，X/Y/Rx/Ry/Rz保持示教结果，避免运行姿态相对示教差90度。
            ...
            for i = 1, PalletNumber.TransPartitionPointNum do
                if (PalletNumber.TeachPoint.TransPartitionPoint.joint[i] ~= nil)
                    ...
                    local PartTransJoint = { joint = {} }
                    PartTransJoint.joint = DeepCopy(PalletNumber.TeachPoint.TransPartitionPoint.joint[i])
                    CData.PartTransTeachJ6[i] = PartTransJoint.joint[6]

                    local PartTransPose = PositiveKin(PartTransJoint,
                        { user = CData.User, tool = CData.Tool.Conc })
                    if (PartTransPose ~= nil) and (PartTransPose.pose ~= nil) then
                        CData.PartTransTeachPose[i] = DeepCopy(PartTransPose.pose)
                    else
