...
---------------------------------------------------------------
-- 打开吸盘。
-- turn on sucker / vacuum
-- 普通箱子和隔板共用入口，通过CPoint.Paras.Mode区分：
-- 1. MotionType.Norm：普通箱子吸附；
-- 2. MotionType.Part：隔板吸附；
-- 3. CPoint.Paras.Sucker=-1表示多吸多次单放；
-- 4. PalletSuckerFunction=-1表示通讯吸盘，其它值表示通用输出吸盘。
    ...
    local SwitchOpenSucker =
    {
        [MotionType.Norm] = function()
            ...
            SetPayload(BoxNum * PalletNumber.BoxProperty.BoxWeight + ToolWeight,
                { CTool[1], CTool[2], 0.5 * (CTool[3] + PalletNumber.BoxProperty.BoxHigh) }) --设置抓取后的负载：工具重量加箱子重量，并同步更新负载质心
        end,
        [MotionType.Part] = function()
            -- 隔板分支：优先使用独立隔板吸盘输出；未启用时再复用普通吸盘或通讯吸盘。
...
---------------------------------------------------------------
-- 关闭吸盘。
-- turn off sucker / release vacuum
-- 本函数同时负责释放物料和更新负载。
-- 普通箱子采用“先打开破真空、延时后再更新负载”的策略，避免箱子还未脱离时提前降低负载引发碰撞或预载误报。
-- 隔板分支保持原有负载更新时机，避免影响隔板动作稳定性。
---------------------------------------------------------------
-- CloseSucker负载更新说明
-- 普通箱体放货时，payload不应该在关闭吸盘前过早更新；如果箱子尚未完全脱离，控制器会认为负载已经变轻，可能触发preload/collision误判。
-- 本项目采用的安全顺序是：先保持旧payload，执行原有release IO，短延时后再SetPayload，最后关闭release并离开放置点。
-- CloseSucker payload update note
-- During normal-box placement, payload should not be updated too early before the box is physically released. Otherwise the controller may think the load is already lighter and trigger preload/collision false alarms.
-- The safer sequence is: keep the old payload, execute the original release IO, wait briefly, update payload with SetPayload, then close release and leave the place point.
---------------------------------------------------------------
local function CloseSucker(PalletNumber, CPoint, CIndex)
    ...
    -- 放货预载优化：普通箱体不再在关闭吸盘前立即更新负载。
    -- 先保持旧负载完成吸盘关闭/破真空打开，延时确认箱子开始脱离后，再更新负载，避免碰撞或预载误报。
    local ReleasePayloadDelay = 200 --破真空打开后延时更新负载的时间，单位毫秒
    local PayloadUpdated = false
    local function UpdatePayloadAfterRelease()
        if PayloadUpdated == false then
            SetPayload(Weight, { CTool[1], CTool[2], 0.5 * CTool[3] }) --根据释放后的剩余箱子数量更新负载和负载质心
            LogInfo("BoxWeight after release delay: %s", Weight)
            PayloadUpdated = true
        end
    end
...
local function TransMotion(CPoint, CDir, Acc, Vel)
    ...
    for i = SD, ED, CDir do
        ...
        if (CDir == Dir.Backward)
            ...
            -- 回程使用src3提供的回程过渡点：J6已经改为终点待机点的J6
            TPoint = CPoint.BackwardMotionPoint[i]
        end

        if (type(TPoint) == "table") then
            -- 普通箱子过渡点和回程过渡点主要用于安全换姿态，不是工艺直线路径。
            -- 普通箱子如果用直线运动经过关节过渡点，当J5接近±90°或J6跨分支时，容易增加点位预处理失败风险。
            -- 隔板去程是实际取隔板后的贴近和放置路径，需要保留直线运动。
            local TAcc = Acc
            local TVel = Vel
            -- 取消过渡点30硬限速：普通箱子和隔板去程/回程都使用调用方传入的界面速度。
            -- 若调用方未传速度，则默认使用空载速度。
            if (TAcc == nil) then
                TAcc = NLDAcc
            end
            ...
            if (CDir == Dir.Backward) then
                if (CPoint.Paras.Mode == MotionType.Part) then
                    -- 隔板回程：从隔板/支架区域退出时保留直线运动，避免关节运动走非直线路径扫到隔板支架
                    -- cp=100：尽量按回程过渡点精确退出，底层更安全。
                    -- J1归一到安全分支，J6选择当前最近且在软限位内的等效角
                    SafeMovL(TPoint, { a = TAcc, v = TVel, cp = 100 }, true)
                else
                    -- 普通箱子回程：不是工艺直线路径，继续用关节运动，降低奇异区和软限位风险
                    -- J1归一到安全分支，J6选择当前最近且在软限位内的等效角
                    SafeMovJ(TPoint, { a = TAcc, v = TVel, cp = 100 }, true)
                end
            else
                if (CPoint.Paras.Mode == MotionType.Part) then
                    -- 隔板去程：取隔板后到放隔板区域必须用直线运动，保持现场示教的贴近和放置效果
                    -- 这里只做J1安全分支和J6软限位检查，不再把J6强行贴近当前角度。
                    SafeMovL(TPoint, { a = TAcc, v = TVel, cp = 100 }, false)
                else
                    -- 普通箱子去程：不是工艺直线贴合路径，继续用关节运动，避免J5接近±90°时直线运动预处理失败
...
local function PTPMotion(PalletNumber, CPoint, CQueue)
    ...
    if (PalletNumber.Mode == WorkType.Pallet) then
        ...
        if (CPoint.Paras.Mode == MotionType.Part) then
            ...
            MovL(CPartPick, { a = NLDAcc, v = NLDVel, stopcond = "AI(1) <= 1.5" }) --运动到抓取点，速度使用界面空载速度
        ...
        for i = 1, CPoint.Paras.Times do
            if CPoint.Paras.Mode == MotionType.Part then
                ...
                if CPoint.Paras.OffSet[i] == 1 then
                    ...
                    MovL(CPoint.MotionPoint[11 + i], { a = PlaceAcc, v = PlaceSpeed, cp = 0 }) --运动到放置点正上方，贴近box需精确到达
                else
                    MovL(CPoint.MotionPoint[11 + i], { a = LDAcc, v = LDVel, cp = 0 }) --运动到放置点正上方，贴近box需精确到达
                end
                MovL(CPoint.MotionPoint[7 + i], { a = PlaceAcc, v = PlaceSpeed, cp = 0 }) --运动到放置点，放货最后一步需精确到达
                CloseSucker(PalletNumber, CPoint, i)
                if SimulateMode == 1 then
                    UpdatePlaceBoxState(PalletNumber, CPoint)
                end
                ...
                MovL(CPoint.MotionPoint[11 + i], { a = NLDAcc, v = NLDVel, cp = 0 }) --放货后垂直抬回正上方，避免剐蹭已放box

                local DepositLiftHeight = 250

                if i < CPoint.Paras.Times then
                    -- 还有下一个箱子
                    -- 在当前到达点基础上再抬高，避免下一次移动时剐蹭已放箱子。
                    ...
                    MovL(liftJointPoint, { a = NLDAcc, v = NLDVel, cp = 0 }) --多次放货之间继续垂直抬升，避免剐蹭
                end
