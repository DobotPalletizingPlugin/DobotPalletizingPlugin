...
local LSPick = { pose = {} }     --隔板取料点
local LiftingPoint = {}          --升降柱安全点位
local NearJoint = { joint = {} } --逆解参考点
...
local function SetCoordinate(PalletNumber, PalletNum)
    ...
    if (PalletNumber.LayerSheet.Enable == true) then
        ...
        if (ProjectType == 1) then
            ...
            LSPick = PositiveKin(LSPickJoint)
        else
            LSPick.pose = DeepCopy(PalletNumber.TeachPoint.TeachLayerSheetPickPoint.pose)
            LSPick.pose[3] = LSPick.pose[3] - PalletNumber.ProcessNum.PalletHeight
        end
    end
end
---------------------------------------------------------------
...
local function InitLifting()
    ...
    local SwitchCommInitMode =
    {
        ...
        [LiftingType.GeMinG] = function()
            ...
            SV660CInit()
            SV660CEnable(1)
            CurrentHeight = SV660CGetPostion()
...
local function OpenVacuumCup(PalletNumber, CPoint, CIndex)
    ...
    if switch_mode then
        ...
        LogInfoTable("[OpenVacuumCup] Center of gravity: ", ToolData)
...
local function CloseVacuumCup(PalletNumber, CPoint, CIndex)
    ...
    LogInfoTable("[CloseVacuumCup] Center of gravity: ", ToolData)
...
local function InitRobot()
    ...
    if (ResetPathFunc == true) then
        if (CPose.pose[1] > 400) then
            ...
            MovL(HomeTransPointL, { v = NLDVel * 0.25, cp = 100 })
        elseif (CPose.pose[1] < -400) then
            HomeTransPointR.joint[6] = 0.5 * CJoint.joint[6]
            MovL(HomeTransPointR, { v = NLDVel * 0.25, cp = 100 })
        end
    end
...
local function StandyMotion(PalletNumber, CPoint)
    if (PalletNumber.State.StateReady == false) and (LiftingHeight > 1) then
        MovJ(CPoint.Paras.Standy, { v = NLDVel, cp = 100 })
    ...
    elseif (CPoint.Paras.Mode == MotionType.LayerSheet) then
        ...
        local ErrID = 0
        local CJoint = { joint = {} }
        ErrID, CJoint = InverseKin(Standy, { jointNear = NearJoint, useJointNear = true })
        if (ErrID ~= 0) then
            LogInfoTable("Standy:", Standy)
            Alarm("Point Unreachable!", ErrorMessage.Type.PointErr)
        end
        MovJ(CJoint, { v = NLDVel, cp = 100 })
    else
        MovJ(CPoint.MotionPoint[7], { v = NLDVel, cp = 100 })
    end
end
---------------------------------------------------------------
--MotionPoint[index]:
...
local function PTPMotion(PalletNumber, CPoint)
    ...
    if (PalletNumber.Mode == WorkType.Pallet) then
        Wait(Time.Pick.Pre)
        if (CPoint.Paras.Mode == MotionType.LayerSheet) then
            local CLSPick = DeepCopy(LSPick)
            local NLDLSVel = MotionParas.Backward.Pick.LSVel
            --MovJ(CPoint.MotionPoint[7], { v = SyncMotionVel, cp = 100 })
            NearJoint = GetAngle()
            ...
            MovL(CPoint.MotionPoint[6], { v = NLDVel, cp = 100 }) --运动到抓取点
        end
        ...
        if (CPoint.Paras.Mode == MotionType.LayerSheet) then
            ...
            MovL(CPoint.MotionPoint[7], { v = LDVel, cp = 100 }) --运动到抓取点上方
        end
        ...
        for i = 1, CPoint.Paras.Times do
            if CPoint.Paras.OffSet[i] == 1 then
                if (CPoint.Paras.Times > 1 and LabelFunction == true) then
                    MovJ(CPoint.MotionPoint[15 + i], { v = LDVel, cp = 100 })  --运动到放置过渡点
                else
                    MovL(CPoint.MotionPoint[15 + i], { v = LDVel, cp = 100 })  --运动到放置过渡点
                end
                MovL(CPoint.MotionPoint[11 + i], { v = LDPlaceVel, cp = 100 }) --运动到放置点正上方
            else
                if (CPoint.Paras.Times > 1 and LabelFunction == true) then
                    MovJ(CPoint.MotionPoint[11 + i], { v = LDVel, cp = 100 }) --运动到放置点正上方
                else
                    MovL(CPoint.MotionPoint[11 + i], { v = LDVel, cp = 100 }) --运动到放置点正上方
                end
            end
            MovL(CPoint.MotionPoint[7 + i], { v = LDPlaceVel, cp = 100 })
            ...
            if (CPoint.Paras.Times > 1 and i == 1) then
                MovL(CPoint.MotionPoint[11 + i], { v = LDPlaceVel, cp = 100 })     --运动到放置点正上方
                if CPoint.Paras.OffSet[i] == 1 then
                    MovL(CPoint.MotionPoint[15 + i], { v = LDPlaceVel, cp = 100 }) --运动到放置过渡点
                end
            else
                MovL(CPoint.MotionPoint[11 + i], { v = NLDVel, cp = 100 }) --运动到放置点正上方
            end
        end
        ...
        if (SingleMotion == false) or (SyncSignal == true)
            ...
            if SingleMotion == false then
                ...
                Standy.pose = DeepCopy(CPoint.Paras.Standy.pose)
                Standy.pose[3] = Standy.pose[3] - CPoint.Paras.LH
                MovJ(Standy, { v = SyncMotionVel, cp = 100 })
            end
            SyncMotion(CPoint.Paras.LH)
        end
        ...
        for i = CPoint.Paras.Times, 1, -1 do
            if (i == CPoint.Paras.Times) then
                if (CPoint.Paras.OffSet[i] == 1) then
                    MovJ(CPoint.MotionPoint[15 + i], { v = NLDVel, cp = 100 }) --运动到放置过渡点
                end
                MovJ(CPoint.MotionPoint[11 + i], { v = NLDVel, cp = 100 })     --运动到放置点正上方
                MovL(CPoint.MotionPoint[7 + i], { v = NLDVel, cp = 100 })
            else
                if (CPoint.Paras.OffSet[i] == 1) then
                    MovJ(CPoint.MotionPoint[15 + i], { v = LDVel, cp = 100 }) --运动到放置过渡点
                end
                MovJ(CPoint.MotionPoint[11 + i], { v = LDVel, cp = 100 })     --运动到放置点正上方
                MovL(CPoint.MotionPoint[7 + i], { v = LDVel, cp = 100 })
            end
        ...
        CPose = GetPose()
        if (CPoint.Paras.Mode == MotionType.LayerSheet) then
            LSSafeMotion(PalletNumber.LayerSheet.SafePoint, Dir.Backward, CPoint.Paras.LH, LDVel)
            local CLSPick = DeepCopy(LSPick)
            CLSPick.pose[3] = CLSPick.pose[3] - CPoint.Paras.LH +
                PalletNumber.ProcessNum.LayerSheetHeight * PalletNumber.LayerSheet.ReLSNum
            MovL(CLSPick, { v = LDVel, cp = 100 }) --运动到放料点
        else
            MovJ(CPoint.MotionPoint[7], { v = LDVel, cp = 100 })
            MovL(CPoint.MotionPoint[6], { v = LDVel, cp = 100 }) --运动到放料点
        end
        ...
        if (CPoint.Paras.Mode == MotionType.LayerSheet) then
            LSSafeMotion(PalletNumber.LayerSheet.SafePoint, Dir.Forward, CPoint.Paras.LH, NLDVel)
        else
            MovL(CPoint.MotionPoint[7], { v = NLDVel, cp = 100 }) --运动到抓取点上方
        end
        StandyMotion(PalletNumber, CPoint)
    end
