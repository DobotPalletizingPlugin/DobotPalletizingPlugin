...
RemoteMode = 0                                   --远程模式 0：非远程模式，1：远程模式
...
ToggleSignalFunc = false        --吸盘信号翻转功能，false: (ON：打开吸盘，OFF: 关闭吸盘),true：(OFF：打开吸盘，ON: 关闭吸盘)
PalletVacuumCupFunc = 0                          --多吸盘功能, -1：通讯控制单吸, 1：通用单吸， 2：通用双吸, 3：通用三吸，4：通用四吸
...
LabelFunction = false           --标签功能，false: 关闭，true：开启
---------------------------------------------------------------------------------------------
MotionParas =                   --运动参数
{
    Forward =                   --带料后速度
...
local function CreatePalletTemplate(CPallet, NearJoint)
    return {
        ...
        ProcessNum =
        {
            ...
            LayerSheetNum = 10,       --隔板最大数量（上位机配置）
            LayerSheetHeight = 0,     --隔板高度（上位机配置）
            LayerSheetWeight = 2      --隔板重量（上位机配置）
        },
        ...
        TeachPoint = {
            ...
            TeachPickPoint =
            {
                pose = {},
                joint = {}
            }, --示教点，码垛：箱体抓取点, 拆垛：箱体放置点（上位机配置）

            TeachLayerSheetPickPoint =
            {
                pose = {},
                joint = {}
            }, --示教点，码垛：隔板抓取点, 拆垛：隔板放置点（上位机配置）

            TeachLayerSheetPlacePoint =
            {
                pose = {},
                joint = {}
            } --示教点，码垛：隔板放置点, 拆垛：隔板抓取点（上位机配置）
        },
        ...
        TransLayerSheetPointNum = 1, --隔板过渡点数量
        LayerSheet =                 --隔板配置
        {
            Enable = false,         --隔板功能，false: 关闭，true：开启
            Mode = 0,               --隔板运动标志位，0：常规点位，1: 隔板点位
            Layer = {},             --隔板放置层位置
            PlanType = 0,                                     --隔板取料方式，0：预设路径，1：实时路径
            SafePoint =
            {
                Forward =
                {
        ...
        Coordinate =
        {
            LayerSheetUserNum = 0, --选择隔板的User编号
            UserNum = 0,          --选择的User编号
            ToolNum = 0           --选择的TCP编号
        },
...
FirstPallet.RegisterID =
{
    ...
    ReLSNum = 5024,      --隔板数量寄存器地址
    BoxNum = 5038        --当前层已码垛箱数寄存器地址
}
------------------------------------------------------------------------------------------
--右侧栈板全局变量
------------------------------------------------------------------------------------------
...
SecondPallet.RegisterID =
{
    ...
    ReLSNum = 5025,      --隔板数量寄存器地址
    BoxNum = 5039        --当前层已码垛箱数寄存器地址
}
-------------------------------------------------------------------------------------------
--通讯相关全局变量
-------------------------------------------------------------------------------------------
...
function SV660CGetPostion()
    ...
    if (Data[1] == nil) then
        if (Communication.Lifting.Init == false) then
...
function LINAKGetPostion()
    local Data = GetHoldRegs(Communication.Lifting.Brand.LINAK.Modbus.Id, 8449, 1)
    if (Data[1] == nil) then
        if (Communication.Lifting.Init == false) then
            Alarm("Communication initialization failed!", ErrorMessage.Type.LinkErr)
        else
...
function Alarm(Message, ErrorCode)
    ...
    if (ErrorMessage.Code == ErrorMessage.Type.LayerSheetErr) then
...
---------------------------------------------------------------
--上传隔板计数
function WriteLSNum(PalletNumber)
    if PalletNumber.Pallet == Left then
        SecondPallet.LayerSheet.ReLSNum = FirstPallet.LayerSheet.ReLSNum
    else
        FirstPallet.LayerSheet.ReLSNum = SecondPallet.LayerSheet.ReLSNum
    end
    WriteRobotModbus(FirstPallet.LayerSheet.ReLSNum, FirstPallet.RegisterID.ReLSNum)
    WriteRobotModbus(SecondPallet.LayerSheet.ReLSNum, SecondPallet.RegisterID.ReLSNum)
...
function ReadPalletNum(PalletNumber)
    if StorageMode == 1 then
        ...
        PalletNumber.LayerSheet.ReLSNum = ReadRobotModbus(PalletNumber.RegisterID.ReLSNum)
    else
        local TData = {}
        if PalletNumber.Pallet == Left then
            TData = GetVal("PalletWorkingDataA")
            FirstPallet.LayerSheet.ReLSNum = GetVal("PalletPartNumA")
        else
            TData = GetVal("PalletWorkingDataB")
            SecondPallet.LayerSheet.ReLSNum = GetVal("PalletPartNumB")
        end
    ...
    if (PalletNumber.LayerSheet.Enable == true) then
        ...
        WriteLSNum(PalletNumber)
        if PalletNumber.Pallet == Left then
            FirstPallet.LayerSheet.Place = GetVal("PalletPartPlaceA")
        else
            SecondPallet.LayerSheet.Place = GetVal("PalletPartPlaceB")
        end
    end
    LogInfo(
        "%s pallet working data: LayerCount: %s, NextBoxCount: %s, RemainBoxNum: %s, LayerBoxNum: %s, AddBoxCount: %s, LayerSheetPlace: %s, LayerSheetNum: %s",
        (PalletNumber.Pallet == Left and "Left" or "Right"),
        ...
        PalletNumber.PalletNum.AddBoxCount, PalletNumber.LayerSheet.Place, PalletNumber.LayerSheet.ReLSNum)
end
---------------------------------------------------------------
...
function CommitPalletNum(PalletNumber)
    ...
    local LayerBoxNum = 0
    local AddBoxCount = 0
    BoxNum = PalletNumber.PalletNum.NextBoxCount - 1
    if (PalletNumber.PalletNum.LayerCount > 1) then
        for i = 1, PalletNumber.PalletNum.LayerCount - 1 do
            LayerBoxNum, AddBoxCount = GetOddBoxCnt(PalletName, PalletNumber.Pallet, i)
            BoxNum = BoxNum - LayerBoxNum - AddBoxCount
        end
    end
    WriteRobotModbus(BoxNum, PalletNumber.RegisterID.BoxNum)
    if PalletNumber.Pallet == Left then
        SetVal("PalletWorkingDataA", PalletNumber.PalletNum)
        SetVal("PalletPartNumA", PalletNumber.LayerSheet.ReLSNum)
        if SimulateMode == 1 then
        ...
        SetVal("PalletPartNumB", PalletNumber.LayerSheet.ReLSNum)
        if SimulateMode == 1 then
...
function GetEnableStatus()
    if (RemoteMode == 0) then
        ...
        if (MultPalletFunction == true) then
            ...
            if (FirstPallet.State.Enable == true) then
                FirstPallet.StateValue.Enable = 1
...
function GetPalletStatus(PalletNumber)
    ...
    if (PalletNumber.LayerSheet.PlanType == PlanType.OnLine) then
        if (CheckDIRes(PalletNumber.LayerSheet.SafePortB.Mode, PalletNumber.LayerSheet.SafePortB.A) == ON) then
            LogWarn("Insufficient Layer sheet!")
            ErrorMessage.Code = ErrorMessage.Type.LayerSheetSafeErr
            CommitErrorMsg(ErrorMessage.Code)
        end
    end
...
----------------------------------------------------------------
--掉料信号检测
function DropSignalDete(PalletNumber, DeteState, VacuumCupPort, Mode)
    if (Mode == DropType.Norm) then
        if (PalletNumber.LayerSheet.Mode == MotionType.LayerSheet) and (PartCfg.Enable == true) then
            if ((DeteState == OFF) and (CheckDORes(PartCfg.Port.Mode, PartCfg.Port.A) == ON)) then
                ...
                if (DeteState == OFF) and (CheckDORes(PartCfg.Port.Mode, PartCfg.Port.A) == ON) then
                    ...
                    Alarm("LayerSheet Fall Down!", ErrorMessage.Type.DropErr)
                end
            end
        else
            if (ToggleSignalFunc == false) then
                if (DeteState == OFF) and (CheckDORes(VacuumCupCfg.Port.Mode, VacuumCupPort) == ON) then
                    Wait(Time.DropDetection)
                    if (DeteState == OFF) and (CheckDORes(VacuumCupCfg.Port.Mode, VacuumCupPort) == ON) then
                        IORes(VacuumCupCfg.Port.Mode, VacuumCupPort, OFF) --数字输出控制吸盘关闭
                        Alarm("Box Fall Down!", ErrorMessage.Type.DropErr)
                    end
                end
            else
                if (DeteState == OFF) and (CheckDORes(VacuumCupCfg.Port.Mode, VacuumCupPort) == OFF) then
                    Wait(Time.DropDetection)
                    if (DeteState == OFF) and (CheckDORes(VacuumCupCfg.Port.Mode, VacuumCupPort) == OFF) then
                        IORes(VacuumCupCfg.Port.Mode, VacuumCupPort, ON) --数字输出控制吸盘关闭
                        Alarm("Box Fall Down!", ErrorMessage.Type.DropErr)
                    end
                end
            end
        ...
        if (ToggleSignalFunc == false) then
            if (DeteState == ON) and (CheckDORes(VacuumCupCfg.Port.Mode, VacuumCupPort) == ON) then
                PalletNumber.StateValue.Status = StateType.DropBox
                CommitPalletStatus(PalletNumber)
                Alarm("TCP With Box!", ErrorMessage.Type.StartErr)
            end
        else
            if (DeteState == ON) and (CheckDORes(VacuumCupCfg.Port.Mode, VacuumCupPort) == OFF) then
                PalletNumber.StateValue.Status = StateType.DropBox
                CommitPalletStatus(PalletNumber)
                Alarm("TCP With Box!", ErrorMessage.Type.StartErr)
            end
...
function DropDete(PalletNumber, Mode)
    local SwitchDropMode =
    {
        ...
        [1] = function() --光电检测
            ...
            DropSignalDete(PalletNumber, ToolDI(VacuumCupCfg.Dete.PE.A), VacuumCupCfg.Port.A, Mode)
            if (PalletVacuumCupFunc == VacuumCupCfg.Type.Single) then
                return
            end
            DropSignalDete(PalletNumber, ToolDI(VacuumCupCfg.Dete.PE.B), VacuumCupCfg.Port.B, Mode)
            if (PalletVacuumCupFunc == VacuumCupCfg.Type.Double) then
                return
            end
            DropSignalDete(PalletNumber, ToolDI(VacuumCupCfg.Dete.PE.C), VacuumCupCfg.Port.C, Mode)
            if (PalletVacuumCupFunc == VacuumCupCfg.Type.Triple) then
                return
            end
            DropSignalDete(PalletNumber, ToolDI(VacuumCupCfg.Dete.PE.D), VacuumCupCfg.Port.D, Mode)
            if (PalletVacuumCupFunc == VacuumCupCfg.Type.Quadruple) then
                return
            end
        end,
        [2] = function() --真空检测
            ...
            DropSignalDete(PalletNumber, DI(VacuumCupCfg.Dete.Vacuum.A), VacuumCupCfg.Port.A, Mode)
            if (PalletVacuumCupFunc == VacuumCupCfg.Type.Single) then
                return
            end
            DropSignalDete(PalletNumber, DI(VacuumCupCfg.Dete.Vacuum.B), VacuumCupCfg.Port.B, Mode)
            if (PalletVacuumCupFunc == VacuumCupCfg.Type.Double) then
                return
            end
            DropSignalDete(PalletNumber, DI(VacuumCupCfg.Dete.Vacuum.C), VacuumCupCfg.Port.C, Mode)
            if (PalletVacuumCupFunc == VacuumCupCfg.Type.Triple) then
                return
            end
            DropSignalDete(PalletNumber, DI(VacuumCupCfg.Dete.Vacuum.D), VacuumCupCfg.Port.D, Mode)
            if (PalletVacuumCupFunc == VacuumCupCfg.Type.Quadruple) then
                return
            end
        end
    ...
    local switch_mode = SwitchDropMode[VacuumCupCfg.Dete.Mode]
    if switch_mode then
        switch_mode()
    else
...
---------------------------------------------------------------
--获取信号定义字符串
function GetIOString(Port, State)
    local IOMode = IOType.DI
    local IOString = ""
    local SwitchIOString =
    {
        [IOType.DI] = function() --DI
            IOString = "DI"
        end,
        [IOType.DO] = function() --DO
            IOString = "DO"
        end,
        [IOType.ToolDI] = function() --ToolDI
            IOString = "ToolDI"
        end,
        [IOType.ToolDO] = function() --ToolDO
            IOString = "ToolDO"
        end
    }

    if (Port.Mode == 1) then
        IOMode = IOType.ToolDI
    end
    local switch_mode = SwitchIOString[IOMode]
    if switch_mode then
        switch_mode()
        IOString = IOString .. "(" .. Port.A .. ") == " .. State
        LogInfo("IOString: %s", IOString)
    else
        Alarm("Signal type is wrong!", ErrorMessage.Type.WorkingDataErr)
    end
