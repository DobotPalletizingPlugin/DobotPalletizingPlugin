...
local function DropDetectionAction(TriLight)
    ...
    IORes(VacuumCupCfg.Port.Mode, VacuumCupCfg.Port.B, OFF) --数字输出控制吸盘关闭
    if LayerSheetSignalFunction == true then
        IORes(PartSignalMode, LayerSheetSignal, OFF)        --数字输出控制吸盘关闭
    end
    ErrorMessage.Code = 5
    Alarm("检测到箱子/隔板掉落")
end
...
local function ChildProcessAction(PalletNumber, FinishState)
    ...
    --报警5："检测到箱子掉落"
    if VacuumCupCfg.Dete.Mode > 0 then
        if PalletVacuumCupFunc == 1 then
            if VacuumCupCfg.Dete.Mode == 1 then
                if (PalletNumber.LayerSheet.Enable == true) and (LayerSheetSignalFunction == true) then
                    if (ToolDI(VacuumCupCfg.Dete.PE.A) == OFF) and (CheckIORes(PartSignalMode, LayerSheetSignal) == ON) then
                    ...
                    if (ToolDI(VacuumCupCfg.Dete.PE.A) == OFF) and (CheckIORes(VacuumCupCfg.Port.Mode, VacuumCupCfg.Port.A) == ON) then
                        Wait(Time.DropDetection)
                        if (ToolDI(VacuumCupCfg.Dete.PE.A) == OFF) and (CheckIORes(VacuumCupCfg.Port.Mode, VacuumCupCfg.Port.A) == ON) then
                            DropDetectionAction(PalletNumber.TriLight)
                        end
            ...
            elseif VacuumCupCfg.Dete.Mode == 2 then
                if (PalletNumber.LayerSheet.Enable == true) and (LayerSheetSignalFunction == true) then
                    if (DI(VacuumCupCfg.Dete.Vacuum.A) == OFF) and (CheckIORes(PartSignalMode, LayerSheetSignal) == ON) then
                        Wait(Time.DropDetection)
                        if (DI(VacuumCupCfg.Dete.Vacuum.A) == OFF) and (CheckIORes(PartSignalMode, LayerSheetSignal) == ON) then
                            DropDetectionAction(PalletNumber.TriLight)
                        end
                    ...
                    if (DI(VacuumCupCfg.Dete.Vacuum.A) == OFF) and (CheckIORes(VacuumCupCfg.Port.Mode, VacuumCupCfg.Port.A) == ON) then
                        Wait(Time.DropDetection)
                        if (DI(VacuumCupCfg.Dete.Vacuum.A) == OFF) and (CheckIORes(VacuumCupCfg.Port.Mode, VacuumCupCfg.Port.A) == ON) then
                            DropDetectionAction(PalletNumber.TriLight)
                        end
        ...
        elseif PalletVacuumCupFunc == 2 then
            if VacuumCupCfg.Dete.Mode == 1 then
                if (PalletNumber.LayerSheet.Enable == true) and (LayerSheetSignalFunction == true) then
                    if ((ToolDI(VacuumCupCfg.Dete.PE.A) == OFF) or (ToolDI(VacuumCupCfg.Dete.PE.B) == OFF))
                        and (CheckIORes(PartSignalMode, LayerSheetSignal) == ON) then
                        Wait(Time.DropDetection)
                        if ((ToolDI(VacuumCupCfg.Dete.PE.A) == OFF) or (ToolDI(VacuumCupCfg.Dete.PE.B) == OFF))
                            and (CheckIORes(PartSignalMode, LayerSheetSignal) == ON) then
                            DropDetectionAction(PalletNumber.TriLight)
            ...
            elseif VacuumCupCfg.Dete.Mode == 2 then
                if (PalletNumber.LayerSheet.Enable == true) and (LayerSheetSignalFunction == true) then
                    if ((DI(VacuumCupCfg.Dete.Vacuum.A) == OFF) or (DI(VacuumCupCfg.Dete.Vacuum.B) == OFF))
                        and (CheckIORes(PartSignalMode, LayerSheetSignal) == ON) then
                        Wait(Time.DropDetection)
                        if ((DI(VacuumCupCfg.Dete.Vacuum.A) == OFF) or (DI(VacuumCupCfg.Dete.Vacuum.B) == OFF))
                            and (CheckIORes(PartSignalMode, LayerSheetSignal) == ON) then
...
----------------------------------------------------------------
--获取到位信号标志位
local function GetBeInpPlaceDIFlag(PalletNumber, VacuumCup, InpPlaceFlag)
    local InpPlaceState
    if PalletNumber.PalletMode == 1 then
        InpPlaceState = ON
    else
...
----------------------------------------------------------------
--多传感器取料信号
local function GetVacuumCupFlag(PalletNumber, PalletNum)
    local OffSet = {}
    local VacuumCup = 0
    if PalletNumber.MultSensorFunction ~= 0 then
        if PalletNumber.PalletMode == 1 and PalletNumber.State.Full == true then
            OffSet, VacuumCup = GetBoxProPerty(PalletName, PalletNum, PalletNumber.ProcessNum.TotalBoxNum)
        else
            if PalletNumber.PalletNum.NextBoxCount < 1 then
                OffSet, VacuumCup = GetBoxProPerty(PalletName, PalletNum, 1)
            elseif PalletNumber.PalletNum.NextBoxCount > PalletNumber.ProcessNum.TotalBoxNum then
                OffSet, VacuumCup = GetBoxProPerty(PalletName, PalletNum, PalletNumber.ProcessNum.TotalBoxNum)
            else
                OffSet, VacuumCup = GetBoxProPerty(PalletName, PalletNum, PalletNumber.PalletNum.NextBoxCount)
            end
        end
    end
...
local function GetBeInpPlaceDI()
    local FirstVacuumCup = 0
    local SecondVacuumCup = 0
    local SwitchConveyor =
    {
        [-1] = function()
            FirstVacuumCup = GetVacuumCupFlag(FirstPallet, 0)
            ...
            GetBeInpPlaceDIFlag(FirstPallet, FirstVacuumCup, 0)
        end,
        [1] = function()
            SecondVacuumCup = GetVacuumCupFlag(SecondPallet, 1)
            ...
            GetBeInpPlaceDIFlag(SecondPallet, SecondVacuumCup, 1)
        end
    }
